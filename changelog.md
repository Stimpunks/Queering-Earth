...
**The same wrong sentence was in three places** — this site’s own build notes, `start-draft` and `publish-draft` — written once and copied twice, and never checked in any of them. That is the drift this register exists to catch, in the files that describe how the register is kept.

**The `noindex` is what makes it survivable, and it is doing more work than we knew.** The orphaned deploy inherited `X-Robots-Tag: noindex` from the drafts base, so it cannot be crawled. **The live risk is a reader rather than a crawler**: anybody holding the old review link keeps reading a superseded draft and may annotate a version that no longer exists. Removing the deploy is now a step in `publish-draft`, and it is a dashboard action — **nothing in this repository can do it and no gate can see it**, which puts it in the same small class as the bookmarklet: a check a person has to run.

CabinetThe recordings list on [/privacy](https://queering.earth/privacy#the-embed) is checked against the facades that exist

**Check 7c.** Section 7b asserts that every third-party *origin* our scripts can reach is named, which is the check that matters for what a browser may talk to and is **structurally blind to how many recordings there are** — one origin covers any number of videos. So it counts the `data-embed-id` facades on every page, reads the list under `#the-embed`, and compares them per page. It also reads the number the page says out loud in its own prose, because a list that grew while the sentence above it did not is the same failure one layer up.

**Made to fail in five shapes before being believed**, each reverted after: the historical bug put back — the two Sheet No. 12 recordings unlisted with the count sentence agreeing with the short list, which is exactly how it shipped; a count word wrong on its own; the list naming a recording no longer on the page; a new facade added to a sheet that had none, which is the case that will actually arrive next; and the two ways the check could be quietly defeated — rewording the sentence it reads, and renaming the section’s `id`. **Both of those fail loudly rather than going silent**, because a gate that stops applying when somebody edits around it is worse than no gate.

Cabinet`check-markup.mjs` stops reporting PASS over a page it never read

**A named target that could not be found printed `SKIP not found` and the run went on to report `PASS`, `0 problem(s)` and exit 0.** It happened here: a page was checked from a branch it did not exist on, and the gate certified it — 0 tags, 0 ids, 0 elements balanced, and a clean bill of health.

**This is the empty-pass fault the rendering gates already guard**, arriving in the one gate that had no guard for it. The wording is taken from theirs, because the reasoning is identical: a page that measures nothing reports zero problems, which is indistinguishable from a clean page in every other line of the output. Unread targets are reported in their own block, never mixed into the problem count — an unmeasured page is a broken run, not a bad page — and under `--check` the run fails.

Proved in four shapes: the missing file alone; a missing file beside a real one, where the real page must still be measured and the run must still fail; without `--check`, where it reports and does not gate, which is this tool’s stated contract; and a real page, which still passes.
...
CabinetThree skills, so drafting never requires a git command

`start-draft` puts a page on the branch and prints the URL to send Helen. `save-draft` commits, pushes and hands the URL back. `publish-draft` takes the file onto `main` — never the branch — and walks the accession checklist this file already states: mount it, card it, log it, file it, stamp it, route it, crumb it, group it, rail it.

`tools/draft.mjs` holds the one thing all three need to agree about, which is **which file is the draft**. Written out three times it would be three answers the day one was edited, in the one place where being wrong means publishing the wrong page. A draft is identified by its marker rather than by a name kept somewhere, so **a page stops being a draft at exactly the moment it is published**, with no state file to fall out of step.

**It matched the bare filename first, which is the third time in one day.** Check 10 shipped that way and fired on this page and on [what is settled](https://queering.earth/what-is-settled); this tool did the same an hour later and reported both of them as drafts in progress. On a site that documents its own build, anything matching a filename matches the prose about that filename — and the discriminator is always the same, because prose escapes the tag or wraps the name in a code span.
...
MountedThe cabinet by date, at [/whats-new](https://queering.earth/whats-new)

**Every index this site had was in the order somebody chose.** [The plate](https://queering.earth/the-plate) runs by accession number, the two lists on the front of the cabinet run by what a page is for, and `tools/pages.mjs` holds that order for the drawers menu, `/llms.txt` and the finding aid’s manifest alike. This register is the one chronological view and its unit is the *accession* — an event — across some forty thousand words. **Nothing answered “what is here, and when did it turn up”,** which is the question a reader who was here last week actually has.

The listing is generated by `tools/make-whats-new.mjs` and written **between markers** into an authored page, the way [the ledger](https://queering.earth/ledger) and [the decisions](https://queering.earth/what-is-settled) are. Star Stuff generates the whole of its equivalent from a template inside the tool and has recorded what that cost: a site-wide pass added a skip link to every page, the next build rebuilt that one from the template, and **the skip link was gone with the tool reporting a clean build.** A generated page does not conflict, it reverts. Edit inside the markers here and the next run takes it back; edit outside them and it survives.

Nothing on it is typed. The summaries are each page’s own `meta description` — the sentence `/llms.txt` already publishes, so there is no third copy and no repetition of the gloss a collection page is forbidden to repeat. The kind and the number come from the plate, which `check-card-order.mjs` already names as the authority for both. The correction counts are read from this register’s own `data-sheet` filing rather than counted by hand.

CabinetThe feed existed, and nothing on the site linked to it

`/feed.xml` has shipped since the agent-readiness pass. It was in every page’s head as `rel=alternate` and on every response as a `Link` header — so a client that had already been handed the URL could find it, and **a person could not.** There was no visible link to it anywhere on the site. A feed with no human twin is a feed for machines only, which is not what a feed is for. [The new page](https://queering.earth/whats-new) is that twin, and carries the first pressable route to either feed.

CabinetOne feed became two, because they are two lists for two readers

The feed carried *this register’s accessions* — new sheets, corrections and housekeeping notes in one stream. On Ryan’s call it is now split. **`/feed.xml` is the pages as they arrive**, the same list as [the new page](https://queering.earth/whats-new) and out of the same pass, so the two cannot be made to say different things. **`/register.xml` is this register**, under the name the site already uses for this page.

**The pages took the old name rather than the other way round**, and that is the part worth writing down: anyone already subscribed stays subscribed to a working feed and simply begins receiving pages, which is the list most people mean when they say “feed”. Every page advertises both, the `Link` header carries both, and `check-metadata.mjs` now fails on a page that names one and not the other.

Label correctedAn accession on this page had no closing tag, and the feed had been dropping one ever since

Found while counting the new feed’s items against the page: **58 accessions on the page, 57 in the feed.** The Bewick accession of 10 September was missing its closing `section` and the two `div`s inside it, so every accession below it was nested one level deeper than it should have been and the file ended one tag short.

**Nothing looked wrong.** A browser closes the tags itself, so the page rendered correctly and has all along; `check-markup.mjs` passed it, because that gate reads parser-rewriting markup, duplicate ids and the single landmark, and does not balance sections. What it cost was invisible in exactly the way this register is organised against: the feed’s regex runs from an opening tag to the first close it finds, so the unclosed accession swallowed the next one whole and **[the accession about every poem arriving in the Markdown mirror as prose](https://queering.earth/changelog#a-2026-09-10-verse-markdown) had never been in the feed at all.**

Closed, and the balance checked both ways: the page now has 58 accessions, the regex finds 58, and the section depth returns to zero at the end of the file. **A gate that balances sections is not written**, and whether it should be is left for the decision log — this is one fault, found by a count that only existed because the feed was being rebuilt.

CabinetThis register cannot say when a page was made, and that was measured rather than assumed

The obvious source for an arrival date was this page, and it does not work. **“Mounted” here means anything added *to* a page, not a page coming into being:** [the colophon](https://queering.earth/design) carries five such entries, [Two Cohabitating Modes](https://queering.earth/two-cohabitating-modes) six, the front of the cabinet ten — and [the privacy page](https://queering.earth/privacy) carries none at all, because nothing was ever mounted on it. Taking the earliest would have dated several pages by accident and one not at all.

So the date is the commit that first added the file, which is a *measurement* of the repository rather than a second hand-kept copy of anything. **And it is gated against what we wrote down.** Eleven sheets carry an accession stamp with a date in it and twelve pages carry a provenance line with another; the generator compares all three for every page and refuses to run on a disagreement, because the day a stamp and a commit stop agreeing this site is about to say two different things about one day. All twelve agree today — checked before a line of the tool was written, not after.

Two things were found by running it rather than by reading it. A page being written has no commit that added it, so an uncommitted page is dated today, loudly, and corrects itself on the next run — self-healing rather than an override table, which would have had to be right forever. And the first version of that fallback used UTC and dated this page **12 September while it was still the 11th at the desk**, a day ahead of the accession you are reading.
...
### [What has arrived, and when](https://queering.earth/whats-new) the arrivals list
