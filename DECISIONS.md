...
Recorded because two sessions worked this repo on one afternoon and the near-miss is
reproducible, not because anything is broken now.
...
- **A worktree is a snapshot, so `grep` finding nothing means "not in my snapshot", never "not
  in the repo".** Before reporting a file, a rule or an entry absent, fetch. This is the
  register's `id="latest"` fault in a different file: *both sessions were locally correct and
  the page was globally wrong.*
...
guard. The protection is the two rules above and a reader.
